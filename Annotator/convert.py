...
def Convert2Yolo(mypath, outpath, project, classes, imageDir):
    ...
    for txt_name in txt_name_list:
        ...
        if not os.path.exists(outpath):
            os.mkdir(outpath)
        txt_outpath = os.path.join(outpath, re.split('.jpg|.png|.bmp|.jpeg',txt_name)[0])
        ...
        for line in lines:
            ...
            if(len(elems) >= 2):
                ...
                # Iterate through the list and replace values less than 0 with 0 and greater than 1 with 1
                ...
                bb_normalized = tuple(bb_list)
                
                txt_outfile.write(str(cls_id) + " " + " ".join([str(a) for a in bb_normalized]) + '\n')
